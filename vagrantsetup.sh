...
# there is a systemd unit dependency ordering issue between etcd and kubernetes api server in this repo
...
# systemd chooses (visible in the journal) to not start kube-apiserver to resolve this loop. We remove the
# After=multi-user.target in etcd (which doesn't makes sense for a wanted by) and override the unit by
# putting a new one in /etc
sed 's/After=multi-user.target//' /usr/lib/systemd/system/etcd.service > /etc/systemd/system/etcd.service

# create ServiceAccount admission controller keys missing when setting up kubernetes packages
